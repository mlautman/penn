...
dir_input = '../data/letter_logs1Justin2/';
dir_output = '../clf_logs/';
prefix = 'proc_';
...
folds = 10;
interp_size = round(logspace(1,3,5));


% SVM Specific Parameters
svm_Cs = [0.2,1,5];                         % regularization
kernels = {'rbf','sigmoid','poly','linear'};% kernels
max_iter = 1E5;                             % max iterations
svm_tol = 10.^(-5);                         % stop tolerance 

gamma = 1./(2*([0.5,1,2,4]).^2);             % kernel width
degree = [2,3,4,7,10];                      % poly kernel degree
